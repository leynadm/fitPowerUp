...
export const usePresetWorkoutsData = () => {
  ...
  const [presetRoutinesData, setPresetRoutinesData] = useState<IPresetWorkoutData[]>([]);
  ...
  const fetchPresetWorkoutsData = async () => {
    ...
    const userPresetRoutinesDataDocRef = doc(userCollectionRef, "userPresetRoutines");

    const preselectedWorkoutsRef = ref(storage, "assets/files/presetWorkoutsData.json");
    const preselectedRoutinesRef = ref(storage, "assets/files/presetRoutinesData.json");

    try {
      const preselectedWorkoutsUrl = await getDownloadURL(preselectedWorkoutsRef);
      const preselectedRoutinesUrl = await getDownloadURL(preselectedRoutinesRef);
      ...
      const preselectedRoutinesParsedJSON = await preselectedRoutinesResponse.json();

      const presetWorkoutsDataDocSnap = await getDoc(userPresetWorkoutsDataDocRef);
      const presetRoutinesDataDocSnap = await getDoc(userPresetRoutinesDataDocRef);
      ...
      const workoutsArray = Object.values(queriedUserPresetWorkoutsData);
      const routinesArray = Object.values(queriedUserPresetRoutinesData);
