function calculateWorkoutsPerYear(selectedGraph:string, selectedExercise:string, timeframe:string, setInitialRawData:any) {
    ...
    request.onsuccess = (event) => {
      ...
      getDataRequest.onsuccess = () => {
        ...
        // Create a map to store unique years and corresponding workout count
        const yearWorkoutsMap = new Map();
        ...
        data.forEach((item:any) => {
          ...
          // Add the workout date to the map for the current year
          if (!yearWorkoutsMap.has(yearKey)) {
            yearWorkoutsMap.set(yearKey, new Set());
          }
          yearWorkoutsMap.get(yearKey).add(currentDate.toDateString());
        });
  
        const sortedYears = Array.from(yearWorkoutsMap.keys())
          .map((yearKey) => {
        ...
        const workoutCountValues = sortedYears.map((year) => yearWorkoutsMap.get(year.start.toISOString()).size);
  ...
  export default calculateWorkoutsPerYear;
