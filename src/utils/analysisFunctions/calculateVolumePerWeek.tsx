...
function calculateVolumePerWeek(selectedGraph:string, selectedExercise:string, timeframe:string, setInitialRawData:any,chartType:string) {
    ...
    request.onsuccess = (event) => {
      ...
      getDataRequest.onsuccess = () => {
        ...
        // Create a map to store unique weeks and corresponding volume
        ...
        data.forEach((item:any) => {
          ...
          // Add the volume to the map for the current week
          if (!weekVolumeMap.has(weekKey)) {
            weekVolumeMap.set(weekKey, 0);
          }
          weekVolumeMap.set(weekKey, weekVolumeMap.get(weekKey) + volume);
        });
  
        const sortedWeeks = Array.from(weekVolumeMap.keys())
          .map((weekKey) => {
